...
coverage: ## check code coverage quickly with the default Python
	poetry run coverage run --source sample_prj -m pytest
	poetry run coverage report -m
	poetry run coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/sample_prj.rst
	rm -f docs/modules.rst
	poetry run sphinx-apidoc -o docs/ sample_prj
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
dist: clean ## builds source and wheel package as well as stand-alone executable
	poetry build
	poetry run pyinstaller sample_prj/cli.py -n sample_prj --onefile
	ls -l dist
